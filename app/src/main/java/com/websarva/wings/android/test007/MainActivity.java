...
import org.jtransforms.fft.DoubleFFT_1D;        // http://tiro105.hateblo.jp/entry/2015/03/02/141526
    ...
    private class ReceiveSound extends AsyncTask<Integer, Integer, Integer> {
        ...
        protected Integer doInBackground(Integer... arryInt) {
            ...
            while (!fRecordingStopPlz) {
                ...
                publishProgress(frequency);                     // AsyncTaskの経過報告を使う

                try {
                    Thread.sleep(1000);                 // もっと良いやり方が？
        ...
        protected void onProgressUpdate(Integer... progress) {  // publishProgress()が実行されると呼び出される
            ...
            setTvStat(String.valueOf(progress[0]));
        }
